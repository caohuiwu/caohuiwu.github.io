...
**系统架构**：
- 完整技术系统，整合硬件、软件、网络等组件协同工作。系统拓扑图、部署架构图等。


# 二、什么是企业架构？
企业架构是组织级的顶层设计框架，涵盖业务战略、流程、数据、应用系统和技术基础设施的全局规划。其核心是确保IT与业务目标的一致性，并协调跨部门的资源整合。

`TOGAF（The Open Group Architecture Framework）`是由国际组织 `The Open Group` 指定的**企业架构方法论**。 框架将企业架构分为**业务、应用、数据和技术**四大核心架构，形成完整的战略蓝图。
- `AMD（Architecture Development Method）`是 `TOGAF` 的开发方法。
...
## 3.6、合成复用原则（Composite Reuse Principle，CRP）
...
观察者模式使用2个类
- `Subject`：主题，提供了`addObserver(), deleteObserver(), notifyObservers()`等方法

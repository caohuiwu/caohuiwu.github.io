...
- 更新表数据的时候，也是如此，发现 `Buffer Pool` 里存在要更新的数据，就直接在 `Buffer Pool` 里更新。然后会把“在某个数据页上做了什么修改”记录到重做日志缓存（`redo log buffer`）里，接着刷盘到 `redo log` 文件里。
  ![redo](2020-08-24-mysql-redo日志/redo.png)
...
![redo_log_文件组](2020-08-24-mysql-redo日志/redo_log_文件组.png)
...
我们还是画一个图进行说明吧：
![redo_log_结构](2020-08-24-mysql-redo日志/redo_log_结构.png)

#### 4.2.1.1. ib_logfile的 前4个 Block
**前4个 block** 存的都是啥管理信息呢？
![前4block](2020-08-24-mysql-redo日志/block_前4.png)
...
#### 4.2.1.2. block结构
每个 `Block` 的结构如下：
![block_结构](2020-08-24-mysql-redo日志/block_结构.png)
...
![redo_log_buffer_结构](2020-08-24-mysql-redo日志/redo_log_buffer_结构.png)
...
```
![mtr_t_内存结构](2020-08-24-mysql-redo日志/mtr_t_内存结构.png)
...
## 4.3. redo log的写入过程
![redo_log_写入过程](2020-08-24-mysql-redo日志/redo_log_写入过程.png)
1. 事务执行
...
![redo_log_两阶段提交](2020-08-24-mysql-redo日志/redo_log_两阶段提交.png)
> 在mysql中数据更新的流程是：
